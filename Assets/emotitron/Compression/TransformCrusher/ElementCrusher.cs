...
namespace emotitron.Compression
{
	...
	public class ElementCrusher : Crusher<ElementCrusher>, IEquatable<ElementCrusher>, ICrusherCopy<ElementCrusher>
	{
		...
		/// <param name="bitposition">Write position of the target buffer.</param>
		/// <param name="bcl"></param>
		public void Write(CompressedElement nonalloc, Vector3 v3, ulong[] bytes, ref int bitposition, BitCullingLevel bcl = BitCullingLevel.NoCulling)
		{
			if (!cached)
				CacheValues();
		...
		public void Read(CompressedElement nonalloc, ulong buffer, ref int bitposition, BitCullingLevel bcl = BitCullingLevel.NoCulling)
		{
			...
			{
				CompressedFloat cx = cache_xEnabled ? new CompressedFloat(xcrusher, buffer.Read(ref bitposition, cache_xBits[(int)bcl])) : new CompressedFloat();
				CompressedFloat cy = cache_yEnabled ? new CompressedFloat(ycrusher, buffer.Read(ref bitposition, cache_yBits[(int)bcl])) : new CompressedFloat();
				CompressedFloat cz = cache_zEnabled ? new CompressedFloat(zcrusher, buffer.Read(ref bitposition, cache_zBits[(int)bcl])) : new CompressedFloat();
